...
def test_fresh_machine_offline_smoke_and_security_review_are_release_gates():
    ...
    assert "actions/attest-sbom@c604332985a26aa8cf1bdc465b92731239ec6b9e" in release_workflow
...
def test_offline_frontend_hides_online_feature_entrypoints():
    ...
    assert "ensureFeatureVisibilityObserver" in app_js
    assert "new MutationObserver" in app_js
    assert "querySelector?.('[data-online-feature]')" in app_js
