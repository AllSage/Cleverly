...
const FOLDER_STATE_KEY = 'cleverly-folder-state';
const FOLDER_ORDER_KEY = 'cleverly-folder-order';
...
export async function selectSession(id, { keepSidebar = false } = {}) {
  ...
  try {
    ...
    // URL hash — the user complained that coming back to Cleverly kept
    ...
    if (window.documentModule) {
      ...
      const meta = sessions.find(s => s.id === id);
      const shouldOpen = localStorage.getItem('cleverly-doc-open-' + id) === '1';
...
export function setSortMode(mode) {
  _sortMode = mode || null;
  if (mode) Storage.set('cleverly-session-sort', mode);
  else Storage.remove('cleverly-session-sort');
  renderSessionList();
}
