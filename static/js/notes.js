...
let _searchQuery = '';
let _viewMode = (typeof localStorage !== 'undefined' && localStorage.getItem('cleverly-notes-view')) || 'list'; // 'list' or 'grid'
...
const REMINDER_FIRED_KEY = 'cleverly-notes-reminder-fired';
...
const REMINDER_GLOWED_KEY = 'cleverly-notes-reminder-glowed';
...
const REMINDER_PENDING_HIGHLIGHT_KEY = 'cleverly-notes-reminder-pending-highlight';
const REMINDER_ACTIVE_HIGHLIGHT_KEY = 'cleverly-notes-reminder-active-highlight';
...
const REMINDER_DISMISSED_AT_KEY = 'cleverly-notes-reminder-dismissed-at';
const NOTES_FIRST_OPEN_HINT_KEY = 'cleverly-notes-first-open-hint-v1';
...
function _fireReminder(note) {
  ...
  const showLocal = (body) => {
    if ('Notification' in window && Notification.permission === 'granted') {
      try {
        const n = new Notification(title, { body, tag: 'note-' + note.id, icon: '/static/cleverly-icon.svg?v=20260613' });
...
export function openPanel() {
  ...
  if (viewBtn) {
    ...
    viewBtn.addEventListener('click', () => {
      ...
      try { localStorage.setItem('cleverly-notes-view', _viewMode); } catch {}
      pane.classList.toggle('notes-view-grid', _viewMode === 'grid');
      _setViewLabel();
      requestAnimationFrame(() => _applyMasonry(document.querySelector('#notes-pane .notes-pane-body')));
    });
...
const _DRAFT_PREFIX = 'cleverly-note-draft-';
