...
async function showModelSelector() {
  return new Promise((resolve) => {
    ...
    const _features = window._cleverlyFeatures || {};
    const _modes = [
      { id: 'chat', label: 'Chat', icon: CHAT_ICON },
      { id: 'agent', label: 'Agent', icon: _ICON_AGENT },
    ];
    if (_features.web_search !== false) {
      _modes.push({ id: 'search', label: 'Search', icon: _ICON_SEARCH });
    }
    if (_features.deep_research !== false) {
      _modes.push({ id: 'research', label: 'Research', icon: _ICON_RESEARCH });
    }
    _modes.forEach(m => {
    ...
    const _selKey = 'cleverly-compare-selections-' + (state._compareMode || 'chat');
    let selections = Storage.getJSON(_selKey) || Storage.getJSON('cleverly-compare-selections') || [];
    // Restore synthesis models for search/research
    if (state._compareMode === 'search' || state._compareMode === 'research') {
      const savedSynth = Storage.getJSON('cleverly-compare-synth-' + state._compareMode);
