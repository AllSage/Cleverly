...
function _initModelPickerDropdown() {
  ...
  function _populate(filter) {
    ...
    // Load favorites
    const favs = (function() { try { return JSON.parse(localStorage.getItem('cleverly-model-favorites') || '[]'); } catch { return []; } })();
  ...
  async function _pick(m) {
    ...
    try { document.dispatchEvent(new CustomEvent('cleverly:model-picked', { detail: m })); } catch {}
  ...
  document.addEventListener('cleverly:auto-select-model', async (e) => {
...
export function updateModelPicker() {
  ...
  // SECURITY: deliberately NOT auto-injecting `cleverly-model-favorites[0]`
