...
const _ACTION_RECENTS_KEY = 'cleverly-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'cleverly-user-actions-recent';
    ...
    ctxRing.addEventListener('click', (e) => {
      ...
      if (compactBtn) {
        compactBtn.addEventListener('click', async (e) => {
          ...
          compactRole.textContent = 'Cleverly';
...
export function addMessage(role, content, modelName, metadata) {
  try {
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Cleverly' : shortModel(resolvedModel);
