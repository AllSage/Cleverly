// ============================================
// Cleverly UI — Main Application Orchestrator
...
import codeWorkspaceModule from './js/codeWorkspace.js?v=20260626-operator-console';
import offlineControlModule from './js/offlineControl.js';
import commandCenterModule from './js/commandCenter.js?v=20260626-operator-console';
import commandPaletteModule from './js/commandPalette.js?v=20260626-operator-console';
...
async function _refreshDefaultChat() {
  try {
    ...
    if (d && d.endpoint_url && d.model) {
      _defaultChat = d;
      try { window.__cleverlyDefaultChat = d; } catch (_) {}
      return d;
    }
  } catch (_) {}
  return null;
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  if (_opener) window._cleverlyRouteOpener = _opener;
  ...
  if (modelSortBtn && modelSortDropdown) {
    ...
    modelSortDropdown.querySelectorAll('.sort-option').forEach(opt => {
      opt.addEventListener('click', () => {
        const mode = opt.dataset.sort;
        Storage.set('cleverly-model-sort', mode);
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  const SPLASH_COUNT_KEY = 'cleverly-tool-splash-counts';
  const SPLASH_MAX = 2;
  const _toolSplashes = {
  ...
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Cleverly...');
      }
  ...
  const UI_VIS_KEY = 'cleverly-ui-visibility';
  ...
  (function migrateOldToolbarVis() {
    const OLD_KEY = 'cleverly-toolbar-visibility';
    try {
...
// ============================================
function startCleverlyApp() {
  if (window.__cleverlyAppStarted) return;
  window.__cleverlyAppStarted = true;
  // Set CSS variables
  ...
  if (documentModule) {
    ...
    if (_curSession && localStorage.getItem('cleverly-doc-open-' + _curSession) === '1') {
  ...
  window._cleverlyBtnIcons = { send: _sendIcon, mic: _micIcon, stop: _stopIcon, newChat: _newChatIcon };
  ...
  if (sessionModule) {
    ...
    sessionModule.loadSessions()
      ...
      .finally(() => {
        ...
        if (window._cleverlyRouteOpener) {
          try { window._cleverlyRouteOpener(); } catch (_) {}
          window._cleverlyRouteOpener = null;
        }
      });
  } else {
...
if (document.readyState === 'loading') {
  document.addEventListener('DOMContentLoaded', startCleverlyApp, { once: true });
} else {
  startCleverlyApp();
}
