...
COLLECTION_NAME = "cleverly_tool_index"
...
BUILTIN_TOOL_DESCRIPTIONS: Dict[str, str] = {
    ...
    "code_workspace": "Manage sealed local code repositories under the Cleverly Docker data volume. Create/import workspaces, run the coding-agent workflow, snapshot/restore/export repos, list tree, read/write files, apply unified diffs, run offline test/build commands, inspect git status/diff, and commit changes. Use for Codex-like code editing inside a repo without network access.",
...
class ToolIndex:
    ...
    _KEYWORD_HINTS = {
        ...
        frozenset({"save contact", "add contact", "new contact", "update contact",
            ...
            {"manage_contact"},
        frozenset({"code workspace", "codebase", "repo", "repository",
                   "edit code", "fix code", "run tests", "git diff",
                   "apply patch", "codex-like", "codex like", "software project",
                   "source tree", "import repo"}):
            {"code_workspace"},
