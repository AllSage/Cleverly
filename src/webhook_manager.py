...
def _webhooks_enabled() -> bool:
    ...
    try:
        ...
        logger.warning("Webhook feature check failed; disabling webhook delivery: %s", exc)
        return False
...
class WebhookManager:
    ...
    async def deliver_test(self, webhook_id: str, url: str, encrypted_secret: Optional[str]):
        ...
        await self._deliver(webhook_id, url, decrypted, "webhook.test", {"message": "Test ping from Cleverly"})
    ...
    async def _deliver(self, webhook_id: str, url: str, secret: Optional[str], event: str, payload: dict):
        ...
        headers = {
            ...
            "X-Cleverly-Event": event,
            "User-Agent": "Cleverly-Webhook/1.0",
        }
        if secret:
            sig = hmac.new(secret.encode(), body.encode(), hashlib.sha256).hexdigest()
            headers["X-Cleverly-Signature"] = sig
