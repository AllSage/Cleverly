...
_HF_TOKEN_STATUS_SNIPPET = (
    'if [ -n "$HF_TOKEN" ]; then '
    'echo "[cleverly] HF token: applied"; '
    'else '
    'echo "[cleverly] HF token: NOT SET — gated/private models will be denied. '
...
def setup_cookbook_routes() -> APIRouter:
    ...
    async def generate_cookbook_ssh_key(request: Request):
        ...
        if not key_path.exists():
            ...
            proc = await asyncio.create_subprocess_exec(
                ssh_keygen, "-t", "ed25519", "-N", "", "-C", "cleverly-cookbook", "-f", str(key_path),
    ...
    def _user_shell_path_bootstrap() -> list[str]:
        return [
            'CLEVERLY_USER_SHELL="${SHELL:-}"',
            'if [ -n "$CLEVERLY_USER_SHELL" ] && [ -x "$CLEVERLY_USER_SHELL" ]; then',
            '  CLEVERLY_USER_PATH="$("$CLEVERLY_USER_SHELL" -ic \'printf "__CLEVERLY_PATH__%s\\n" "$PATH"\' 2>/dev/null | sed -n \'s/^__CLEVERLY_PATH__//p\' | tail -n 1 || true)"',
            '  if [ -n "$CLEVERLY_USER_PATH" ]; then export PATH="$CLEVERLY_USER_PATH:$PATH"; fi',
            'fi',
        ]
    ...
    async def model_download(request: Request, req: ModelDownloadRequest):
        ...
        # When Cleverly runs from a venv (e.g. native macOS install), put its bin
        ...
        if remote and is_windows:
            ...
            ps_lines = []
            ps_lines.append('$sessionDir = "$env:TEMP\\cleverly-sessions"')
    ...
    async def model_serve(request: Request, req: ServeRequest):
        ...
        if is_windows and remote:
            ...
            runner_lines = ["#!/bin/bash"]
            runner_lines.extend(_user_shell_path_bootstrap())
            runner_lines.append('CLEVERLY_PREFLIGHT_EXIT=""')
            # Put Cleverly's own venv bin on PATH (local runs only) so the serve
            ...
            if "llama_cpp" in req.cmd or "llama-server" in req.cmd:
                ...
                runner_lines.append('      echo "[cleverly] CUDA nvcc found — building llama-server with CUDA (GPU) support..."')
                runner_lines.append('      cmake -B build -DCMAKE_BUILD_TYPE=Release -DGGML_CUDA=ON \\')
                runner_lines.append('        && cmake --build build -j"$NPROC" --target llama-server \\')
                runner_lines.append('        && ln -sf ~/llama.cpp/build/bin/llama-server ~/bin/llama-server')
                runner_lines.append('    else')
                runner_lines.append('      echo "[cleverly] WARNING: nvcc not found — building llama-server for CPU only."')
                runner_lines.append('      echo "[cleverly]   GPU inference will not be available for this llama.cpp build."')
                runner_lines.append('      echo "[cleverly]   To get a GPU build, first install vLLM via Cookbook -> Dependencies"')
                runner_lines.append('      echo "[cleverly]   (its CUDA wheels include nvcc), then re-launch this serve task."')
                ...
                runner_lines.append('  CLEVERLY_PREFLIGHT_EXIT=127')
                runner_lines.append('fi')
                ...
                runner_lines.append('  CLEVERLY_PREFLIGHT_EXIT=1')
                runner_lines.append('fi')
    ...
    async def server_setup(request: Request, req: SetupRequest):
        ...
        if platform == "windows":
            ...
            setup_script = (
                'powershell -Command "'
                "New-Item -ItemType Directory -Force -Path $env:TEMP\\cleverly-sessions | Out-Null; "
