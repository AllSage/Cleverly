...
DOCKER_IN_CONTAINER_HINT = (
    "Not available inside the Cleverly container by design. The image ships no "
...
TMUX_LOG_DIR = Path(tempfile.gettempdir()) / "cleverly-tmux"
...
async def _generate_tmux(cmd: str, request: Request):
    ...
    script_path.write_text(
        f"#!/bin/bash\n"
        f"CLEVERLY_USER_SHELL=\"${{SHELL:-}}\"\n"
        f"if [ -n \"$CLEVERLY_USER_SHELL\" ] && [ -x \"$CLEVERLY_USER_SHELL\" ]; then\n"
        f"  CLEVERLY_USER_PATH=\"$(\"$CLEVERLY_USER_SHELL\" -ic 'printf \"__CLEVERLY_PATH__%s\\n\" \"$PATH\"' 2>/dev/null | sed -n 's/^__CLEVERLY_PATH__//p' | tail -n 1 || true)\"\n"
        f"  if [ -n \"$CLEVERLY_USER_PATH\" ]; then export PATH=\"$CLEVERLY_USER_PATH:$PATH\"; fi\n"
        f"fi\n"
