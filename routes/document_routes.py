...
def setup_document_routes(session_manager, upload_handler=None) -> APIRouter:
    ...
    async def create_document(request: Request, req: DocumentCreate) -> Dict[str, Any]:
        ...
        try:
            ...
            if req.session_id:
                ...
                if user is not None and session.owner != user:
    ...
    async def import_pdf(
        ...
        if session_id:
            ...
            try:
                ...
                if not sess:
                    raise HTTPException(404, "Session not found")
                if user is not None and sess.owner != user:
    ...
    async def prepare_signed_reply(doc_id: str, request: Request):
        ...
        # as email_routes (CLEVERLY_MAIL_ATTACHMENTS_DIR).
        from pathlib import Path as _Path
        import os as _os
        _DATA_DIR = _Path(__file__).resolve().parent.parent / "data"
        from src.compat import getenv as _compat_getenv
        _BASE = _compat_getenv("CLEVERLY_MAIL_ATTACHMENTS_DIR", str(_DATA_DIR / "mail-attachments"))
