...
def _email_disabled_reason(action: str) -> str:
    if offline_mode():
        return f"Email {action} is disabled in offline mode"
    try:
        if (load_features() or {}).get("email") is False:
            return f"Email {action} is disabled"
    except Exception as exc:
        logger.warning("Email feature check failed; disabling email %s: %s", action, exc)
        return f"Email {action} is disabled"
    return ""
...
def _send_smtp_message(cfg: dict, from_addr: str, recipients: list[str], message: str | bytes, timeout: int = 30) -> None:
    ...
    """
    disabled_reason = _email_disabled_reason("sending")
    if disabled_reason:
...
# Override at deploy time via CLEVERLY_MAIL_ATTACHMENTS_DIR. Defaults to a
# subdir of the install's data/ tree so the app works out-of-the-box without
# a hardcoded /home/<user>/ path.
ATTACHMENTS_DIR = Path(getenv("CLEVERLY_MAIL_ATTACHMENTS_DIR", str(DATA_DIR / "mail-attachments")))
...
def _init_scheduled_db():
    ...
    try:
        ...
        if "cleverly_kind" not in cols:
            conn.execute("ALTER TABLE scheduled_emails ADD COLUMN cleverly_kind TEXT")
    except Exception:
        pass
...
def _imap_connect(account_id: str | None = None, owner: str = ""):
    ...
    disabled_reason = _email_disabled_reason("polling")
    if disabled_reason:
        raise RuntimeError(disabled_reason)
...
class SendEmailRequest(BaseModel):
    ...
    # Internal marker for Cleverly-generated mail (e.g. reminder, scheduled).
    cleverly_kind: Optional[str] = None
